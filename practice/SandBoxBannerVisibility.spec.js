...
async function loginAndGoToEvents(page) {
    ...
    await page.getByPlaceholder("you@email.com").fill("kathy.mutyala@gmail.com");
    await page.getByLabel("Password").fill("Kathy@18");
...
test("Sandbox Banner Visibility Test", async ({ page }) => {
    //step1:Set up the API mock
...
test.only("Sandbox Banner NOT Visible Test", async ({ page }) => {
    //step1:Set up the API mock
    await page.route("**/api/events**",
        async route => {
            await route.fulfill({
